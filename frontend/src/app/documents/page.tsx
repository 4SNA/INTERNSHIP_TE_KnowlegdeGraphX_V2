...
export default function DocumentsPage() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (window.confirm(`Are you sure you want to remove "${name}" from the neural graph?`)) {
  ...
  return (
    <ProtectedRoute>
      <div className="flex flex-col min-h-screen w-full bg-[#050510] font-sans selection:bg-indigo-500/30 overflow-hidden">
        <Navbar />
        <div className="flex flex-1 w-full overflow-hidden">
          ...
          <main className="flex-1 w-full flex flex-row overflow-hidden relative">
            ...
            <div className="flex-1 h-full overflow-y-auto p-8 lg:p-12 no-scrollbar scroll-smooth">
                ...
                <section className="space-y-6">
                  <div className="flex items-center justify-between border-b border-zinc-900 pb-6 uppercase font-extrabold text-[10px] text-zinc-500 tracking-[0.2em]">
                    ...
                    <div className="flex items-center gap-6">
                      <span className="text-indigo-400/60 font-bold">{documents.length} Entities Indexed</span>
                    </div>
                  </div>
            ...
            <aside className="w-80 border-l border-zinc-900/60 bg-zinc-950/20 backdrop-blur-3xl hidden xl:flex flex-col p-8 space-y-10 animate-in slide-in-from-right-10 duration-700">
               
               <div className="space-y-6">
                  <h3 className="text-[10px] uppercase font-extrabold text-zinc-500 tracking-[0.3em] px-1">Storage Analytics</h3>
                  <div className="space-y-4">
                     <div className="p-5 rounded-[28px] bg-zinc-900/40 border border-zinc-800/60 space-y-4">
                        <div className="flex items-center justify-between">
                           <span className="text-xs font-bold text-zinc-400">Memory Load</span>
                           <span className="text-xs font-extrabold text-indigo-400">{formatSize(totalSize)}</span>
                        </div>
                        <div className="h-1.5 w-full bg-zinc-950 rounded-full overflow-hidden p-[1px]">
                           <div 
                            className="h-full bg-indigo-500 rounded-full shadow-[0_0_10px_rgba(99,102,241,1)]" 
                            style={{ width: `${Math.min((totalSize / (1024 * 1024 * 100)) * 100, 100)}%` }} // Max 100MB preview
                           />
                        </div>
                        <div className="flex justify-between text-[10px] font-bold text-zinc-600 uppercase tracking-wider">
                           <span>{documents.length} Nodes</span>
               ...
               <div className="space-y-6">
                  <h3 className="text-[10px] uppercase font-extrabold text-zinc-500 tracking-[0.3em] px-1">Neural Pulse</h3>
                  <div className="space-y-3">
                     <ActivityItem icon={Zap} label="Low-Latency Index" status="Active" color="text-yellow-500" />
                     <ActivityItem icon={ShieldCheck} label="Vector Encryption" status="Verified" color="text-emerald-500" />
                     <ActivityItem icon={Database} label="Knowledge Mirror" status="Synced" color="text-blue-500" />
                  </div>
               </div>

               <div className="mt-auto p-6 rounded-[32px] bg-indigo-500/5 border border-indigo-500/10 space-y-3">
                  <div className="w-10 h-10 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-400">
                     <Server size={20} />
                  </div>
                  <h4 className="text-xs font-bold text-zinc-200">System Integrity</h4>
                  <p className="text-[10px] text-zinc-500 font-medium leading-relaxed uppercase tracking-tighter">
                     All segments are currently distributed across the neural search cluster.
                  </p>
               </div>
...
function ActivityItem({ icon: Icon, label, status, color }: any) {
  return (
    <div className="flex items-center justify-between p-3 rounded-2xl bg-zinc-900/20 border border-transparent hover:border-zinc-800/40 transition-all">
       <div className="flex items-center gap-3">
          <div className={cn("p-2 rounded-lg bg-zinc-900/60", color)}>
             <Icon size={14} />
          </div>
          <span className="text-[11px] font-bold text-zinc-400">{label}</span>
       </div>
       <span className={cn("text-[9px] font-extrabold uppercase tracking-widest", color)}>{status}</span>
    </div>
  );
}
